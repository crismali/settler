(function() {
  ...
  function strictlyTyped(types, suspect) {
    exactly(2, arguments);

    return validateOrWrap(validateStrictlyTyped, strictlyTypedFunction, suspect, arguments);
  }

  function strictlyTypedFunction(types, func) {
    return function() {
      validateStrictlyTyped(types, arguments);
      return func.apply(this, arguments);
    };
  }

  function validateStrictlyTyped(types, args) {
    var type, arg, argType;
    exactly(types.length, args);
    ...
    for (var i = 0; i < types.length; i++) {
      ...
      if (isUndefined(arg)) {
        ...
        argType = 'null';
      } else if (isNotANumber(arg)) {
        argType = 'NaN';
      } else if (arg.constructor !== type) {
        argType = getName(arg.constructor);
      }

      if (argType) {
        throw new Error('Expected ' + getName(type) +
          ', but received ' + argType + ' as the ' +
          inflectedNumber(i + 1) + ' argument');
      }
    }
  }

  function inflectedNumber(number) {
    var inflector = {
      '1': 'st',
      '2': 'nd',
      '3': 'rd'
    };
    return number + (inflector[number] || 'th');
  }

  function getName(func) {
    return func.name || 'Unnamed Type';
  }

  // Anything in this object is publically available
  var Settler = {
    arrayArgs: arrayArgs,
